以下に記載しているのはpythonのstreamlitを使って提出された週刊予定からアルバイトのシフト表を作成するアプリです。
以下の機能を実装したいんですがどうすればいいでしょう。
・各曜日が列のラベルになっていて各列には名前を表示させる
・各曜日に入る人数はneedを用いる
・filtered_status_dfで各曜日にOKが表示された中から人を選ぶ
・needとOKと表示された人の人数の差が小さい曜日から配置していき、各人員が選ばれた回数をそれぞれ記憶して、少ない回数の人から配置をしていく
